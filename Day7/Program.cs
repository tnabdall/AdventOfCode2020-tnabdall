...
namespace Day7
{
    class Program
    {
        ...
        {
            var rules = new Dictionary<string, Dictionary<string, long>>();
            foreach(var line in input.Split('\n', StringSplitOptions.RemoveEmptyEntries))
            {
                var container = line.Split(" bags ")[0];
                var contained = line.Split(" contain ")[1];

                var containedBagsDict = new Dictionary<string, long>();
                if (contained.Contains("no other"))
                {
                    rules[container] = containedBagsDict;
                    continue;
                }

                var containedBags = contained.Split(',');
                foreach(var containedBag in containedBags)
                {
                    var parts = containedBag.Trim().Split(' ');
                    containedBagsDict[parts[1] + " " + parts[2]] = long.Parse(parts[0]);
                }
                rules[container] = containedBagsDict;
            }

            IEnumerable<string> bagsToSearchFor = new List<string>() { "shiny gold" };

            HashSet<string> containableBags = new HashSet<string>();

            while (bagsToSearchFor.Count() > 0)
            {
                bagsToSearchFor = rules
                    .Where(bag => bagsToSearchFor.Any(search => bag.Value.ContainsKey(search)))
                    .Select(bag => bag.Key).Distinct().ToArray();
                foreach (var bag in bagsToSearchFor)
                    containableBags.Add(bag);
            }

            Console.WriteLine(containableBags.Count);
        }
        ...
        private static void Part2(string input)
        {
            ...
            var count = GetSubBagCount(rules["shiny gold"], rules);

            Console.WriteLine(count);
        }

        private static long GetSubBagCount(Dictionary<string, long> bag, Dictionary<string, Dictionary<string, long>> rules)
        {
            if (bag.Count == 0)
                return 0;
            else 
                return bag.Sum(e => e.Value) + bag.Sum(e => e.Value * GetSubBagCount(rules[e.Key], rules));
        }



        const string EXAMPLE_INPUT = @"light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.";
        ...
        const string EXAMPLE_INPUT_2 = @"shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.";
        ...
        const string PROBLEM_INPUT = @"shiny plum bags contain no other bags.
