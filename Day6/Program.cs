...
namespace Day6
{
    class Program
    {
        ...
        {
            var listOfGroups = new List<Dictionary<char, long>>();
            var currentGroup = new Dictionary<char, long>();
            foreach (var line in input.Split('\n', StringSplitOptions.None).Select(e => e.Trim()))
            {
                if (line.Length == 0 && currentGroup.Count > 0)
                {
                    listOfGroups.Add(currentGroup);
                    currentGroup = new Dictionary<char, long>();
                    continue;
                }
                ...
                foreach(var chr in line){
                    AddOrCreate(currentGroup, chr, 1);
                }
            }
            if (currentGroup.Count > 0)
                listOfGroups.Add(currentGroup);

            Console.WriteLine($"Answer is {listOfGroups.Sum(e => e.Count)}");
        }

        private static void Part2(string input)
        {
            var listOfGroups = new List<(Dictionary<char, long> dict, long total)>();
            var currentGroup = new Dictionary<char, long>();
            var currentTotal = 0;
            foreach (var line in input.Split('\n', StringSplitOptions.None).Select(e => e.Trim()))
            {
                if (line.Length == 0 && currentGroup.Count > 0)
                {
                    listOfGroups.Add((currentGroup, currentTotal));
                    currentGroup = new Dictionary<char, long>();
                    currentTotal = 0;
                    continue;
                }
            ...
            Console.WriteLine($"Answer is {listOfGroups.Sum(e => e.dict.Where(g => g.Value == e.total).Count())}");
        }
        ...
        const string EXAMPLE_INPUT = @"abc
...
b";
        ...
        const string PROBLEM_INPUT = @"bdmceunt
ubcdjqvnmte
mcgetfndul
dcenmtu
